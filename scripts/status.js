...
async function checkDatabase() {
	try {
		const mongoUri =
			process.env.MONGO_URI || 'mongodb://127.0.0.1:27017/e_system';
...
async function showStatus() {
	...
	const endpoints = [
		...
		{ name: 'API Docs', path: '/api/v1', expected: 200 },
		{ name: 'Admin (401 expected)', path: '/api/v1/admin', expected: 401 },
	];
	...
	for (const endpoint of endpoints) {
		try {
			...
			if (result.statusCode === endpoint.expected) {
				...
				console.log(
					`   ⚠️ ${endpoint.name}: ${result.statusCode} (expected ${endpoint.expected})`
				);
			}
		} catch (error) {
	...
	console.log(
		'   • Authorization is working (401 responses are normal for protected routes)'
	);
	console.log('\n📋 Quick Links:');
