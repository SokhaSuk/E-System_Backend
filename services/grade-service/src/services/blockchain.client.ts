 ...
 * 
 * Client for communicating with the blockchain service.
 */
...
export class BlockchainClient {
	...
	async getRecord(recordId: string, recordType: string) {
		try {
			const response = await fetch(
				`${this.baseUrl}/api/v1/blockchain/record/${recordId}?recordType=${recordType}`,
				{
					method: 'GET',
					headers: {
						'Content-Type': 'application/json',
